...
const BreedScreen = () => {
  ...
  const fetchBreedings = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/breeding/get-goat-breedings/${userID}`, {
        method: 'GET',
        headers: {
  ...
  const handleEditBreed = async (breedingId) => {
    ...
    navigation.navigate('GoateditBreeding', { breedingId })
  };
  ...
  return (
    ...
    <FAB
        ...
        onPress={() => {
          ...
          navigation.navigate('GoatAddBreeding');
        }}
      />
    </View>
  );
