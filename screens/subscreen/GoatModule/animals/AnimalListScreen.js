...
const AnimalListScreen = () => {
  ...
  const fetchAnimals = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/animal/get-goat-animals/${userID}`, {
        method: 'GET',
        headers: {
  ...
  const handleDeleteAnimal = async (animalId) => {
    ...
    const handleEditAnimal = async (animal) => {
      navigation.navigate('GoatAnimalDetails', { animal });
    };
  ...
  return (
    ...
    <FAB
        ...
        onPress={() => {
          ...
          navigation.navigate('GoatAddAnimal');
        }}
      />
    </View>
  );
