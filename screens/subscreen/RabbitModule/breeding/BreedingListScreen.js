...
const BreedScreen = () => {
  ...
  const fetchBreedings = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/breeding/get-rabbit-breedings/${userID}`, {
        method: 'GET',
        headers: {
  ...
  const handleEditBreed = async (breedingId) => {
    ...
    navigation.navigate('RabbiteditBreeding', { breedingId })
  };
  ...
  return (
    ...
    <FAB
        ...
        onPress={() => {
          ...
          navigation.navigate('RabbitAddBreeding');
        }}
      />
    </View>
  );
