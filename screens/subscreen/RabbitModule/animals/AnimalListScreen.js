...
const AnimalListScreen = () => {
  ...
  const fetchAnimals = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/animal/get-rabbit-animals/${userID}`, {
        method: 'GET',
        headers: {
  ...
  const handleDeleteAnimal = async (animalId) => {
    ...
    const handleEditAnimal = async (animal) => {
      navigation.navigate('RabbitAnimalDetails', { animal });
    };
  ...
  return (
    ...
    <FAB
        ...
        onPress={() => {
          ...
          navigation.navigate('RabbitAddAnimal');
        }}
      />
    </View>
  );
