...
const MilkScreen = () => {
  ...
  const fetchMilkings = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/milk/get-rabbit-milkings/${userID}`, {
        method: 'GET',
        headers: {
  ...
  const handleEditMilking = async (milkingID) => {
    ...
    navigation.navigate('RabbiteditMilk', { milkingID })
  };
  ...
  return (
    ...
    <FAB
        ...
        onPress={() => {
          ...
          navigation.navigate('RabbitAddMilk');
        }}
      />
    </View>
  );
