...
const BreedScreen = () => {
  ...
  const fetchBreedings = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/breeding/get-cow-breedings/${userID}`, {
        method: 'GET',
        headers: {
  ...
  const handleEditBreed = async (breedingId) => {
    ...
    navigation.navigate('CoweditBreeding', { breedingId })
  };
  ...
  return (
    ...
    <FAB
        ...
        onPress={() => {
          ...
          navigation.navigate('CowAddBreeding');
        }}
      />
    </View>
  );
