...
const BreedScreen = () => {
  ...
  const fetchBreedings = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/breeding/get-pig-breedings/${userID}`, {
        method: 'GET',
        headers: {
  ...
  const handleEditBreed = async (breedingId) => {
    ...
    navigation.navigate('PigeditBreeding', { breedingId })
  };
  ...
  return (
    ...
    <FAB
        ...
        onPress={() => {
          ...
          navigation.navigate('PigAddBreeding');
        }}
      />
    </View>
  );
